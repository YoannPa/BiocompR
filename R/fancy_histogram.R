...
fancy.hist <- function(
    ...
    if(is.numeric(x)){
        ...
        #Set graduations
        xgrads <- round(
            x = seq(0, xmax, length.out = ngrad), digits = round.grad)
        #Set X axis labels
        ...
        histlim <- seq(0, xmax, length.out = ngrad)*(
            length(histdata)/xmax) + 0.5
        ...
        if(!is.null(facet)){
            ...
            if(length(uniq.x) != length(facet)){
                stop(paste("facet must be of the same length as unique(x) to",
                           "be applied."))
            } else {
                #Make dt.facet
                dt.facet <- data.table::data.table(
                    uniq.x = uniq.x, facet = facet)
                #Get facet sizes
                ...
                ls.xbreaks <- lapply(
                    X = reduce.dt.facet$facet, FUN = function(i){
                ...
                if(verbose){ cat("Computing quantities in each bin...\n") }
                ls.histdata <- lapply(
                    X = seq_along(ls.xbreaks), FUN = function(i){
                        if(verbose){
                            cat(paste("\t", names(ls.xbreaks)[i], "\n"))
                        }
                        if(length(ls.xbreaks[[i]]) == 1){
                            sub.chars <- dt.facet[
                                facet == names(ls.xbreaks)[i]]$uniq.x
                            histdata <- list(length(x[x %in% sub.chars]))
                        } else{
                            histdata <- parallel::mclapply(X = seq(
                                length(ls.xbreaks[[i]])-1), mc.cores = ncores,
                                FUN = function(j){
                                    if(j == length(ls.xbreaks[[i]])-1){
                                        #If last bin take values equal to
                                        # maximum too
                                        sub.chars <- dt.facet[facet == names(
                                            ls.xbreaks)[i]]$uniq.x[round(
                                                ls.xbreaks[[i]][j]):round(
                                                    ls.xbreaks[[i]][j+1])]
                                    } else {
                                        sub.chars <- dt.facet[facet == names(
                                            ls.xbreaks)[i]]$uniq.x[round(
                        ...
                        data.table::data.table(
                            x = seq(histdata), y = unlist(histdata))
                    })
                names(ls.histdata) <- reduce.dt.facet$facet
                dframe <- data.table::rbindlist(
            ...
            if(verbose){ cat("Computing quantities in each bin...") }
            histdata <- parallel::mclapply(
                X = seq(length(xbreaks)-1), mc.cores = ncores,
                FUN = function(i){
                    if(i == length(xbreaks)-1){
                        #If last bin take values equal to maximum too
                        sub.chars <- uniq.x[
                            round(xbreaks[i]):round(xbreaks[i+1])]
                    } else {
                        sub.chars <- uniq.x[
                            round(xbreaks[i]):round(xbreaks[i+1]-1)]
                    }
                    length(x[x %in% sub.chars])
                })
            histdata <- unlist(histdata)
        ...
        if(cutoff.val <= median.val){
            gghist <- gghist +
                ...
                ggrepel::geom_label_repel(
                    data = data.frame(),
                    ggplot2::aes(
                        x = cutoff.pos, y = Inf, fontface = 2, label = paste0(
